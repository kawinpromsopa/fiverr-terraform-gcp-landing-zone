...
resource "google_compute_instance_from_template" "default" {
  ...
  name     = "${var.instance_name}-${format("%03d", count.index + 1)}"
  project  = local.project_id
  zone     = data.google_compute_zones.default.names[count.index % length(data.google_compute_zones.default.names)]
